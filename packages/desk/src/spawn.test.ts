...
import { deskWorkerExitAfterTurn, deskWorkerLaunch } from "./spawn.js";
...
test("This Computer exits after a turn; Remote tools workers stay resident", () => {
  assert.equal(deskWorkerExitAfterTurn("all"), "1");
  assert.equal(deskWorkerExitAfterTurn(undefined), "1");
  assert.equal(deskWorkerExitAfterTurn("tools"), "0");
});
