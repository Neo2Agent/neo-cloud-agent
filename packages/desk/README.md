...
本地 `pnpm dev` / `pnpm dev:desk` 只连本机控制面（内存事件总线 + 本地 Run）。要和线上 MySQL / Redis / VM 槽是同一条总线，用 `pnpm dev:desk:prod`（默认 `https://neorun.cloud`，443 不通时回落 `http://62.234.211.200`）。Web 看现网直接打开该地址，不必再起一份本地 Web。
