...
test("session tools include filesystem tools plus neo-git, neo-pr, and neo-diag", () => {
  ...
  assert.match(CLOUD_SYSTEM_PROMPT, /neo_memory_update/);
  assert.match(CLOUD_SYSTEM_PROMPT, /neo_memory_delete/);
  ...
  assert.equal(sessionToolNames({ includeSubagent: false }).includes("neo_memory_update"), true);
  assert.equal(sessionToolNames({ includeSubagent: false }).includes("neo_memory_delete"), true);
});
