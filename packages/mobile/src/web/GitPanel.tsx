...
async function api(token: string, url: string, options: RequestInit = {}): Promise<Response> {
...
function formatListWhen(value: string, now = new Date()): string {
...
function Icon({ children, size = 12 }: { children: string; size?: number }) {
  return <span aria-hidden="true" style={{ width: size, height: size, display: "inline-grid", placeItems: "center", fontSize: size }}>{children}</span>;
}
const IconBranch = ({ size }: { size?: number }) => <Icon size={size}>⎇</Icon>;
const IconCheck = ({ size }: { size?: number }) => <Icon size={size}>✓</Icon>;
const IconChevronDown = ({ size }: { size?: number }) => <Icon size={size}>▾</Icon>;
const IconCopy = ({ size }: { size?: number }) => <Icon size={size}>⎘</Icon>;
const IconError = ({ size }: { size?: number }) => <Icon size={size}>!</Icon>;
const IconMore = ({ size }: { size?: number }) => <Icon size={size}>⋯</Icon>;
const IconSpinner = ({ size }: { size?: number }) => <Icon size={size}>◌</Icon>;
    ...
    <header className="git-head">
      <div className="git-head-title">
        {pr ? (
          ...
          <span className="git-pr-name is-muted">{context === "desk" ? "Desk 上的改动" : "还没有 PR"}</span>
        )}
  ...
  return (
    <div className="git-diff">
      ...
      {diff?.source === "desk" && diff.capturedAt ? (
        <p className="git-note">来自 Desk 的快照 · {formatListWhen(diff.capturedAt)}前</p>
      ) : null}
      {context === "desk" && diff?.source === "none" ? <p className="git-note">正在等 Desk 上报改动。Desk 离线时这里是空的。</p> : null}
      {files.length === 0 ? (
      ...
      {context === "cloud" && dirty ? (
        ...
        <p className="git-note git-commit-note">本机对话的改动在 Desk 上提交，这里只读。</p>
      ) : null}
    </div>
  );
}
