...
- Production default is `AGENT_KERNEL=pi` (colocated worker loop). `deploy.sh` writes `AGENT_KERNEL=pi` and disables `neo-loop`. Do not put `:8082` in Caddy or the Lighthouse firewall. Pass `kernel: "agentscope"` or `AGENT_KERNEL=agentscope` plus a running `neo-loop` to use Java.
