...
- Production default is `AGENT_KERNEL=agentscope` (Java `neo-loop` on `:8082`). `deploy.sh` installs Java 21 if missing, writes `AGENT_KERNEL` / `NEO_LOOP_URL`, and `enable --now neo-loop`. Do not put `:8082` in Caddy or the Lighthouse firewall. Pass `kernel: "pi"` or `AGENT_KERNEL=pi` to keep the colocated worker.
