...
test.describe("voice boundaries", () => {
  test("speech.not-configured: mic click surfaces the missing-key copy", async ({ page }) => {
    await loginAs(page);
    const mic = page.locator("#composer button.composer-mic, #composer button[aria-label='点一下开始说话']");
    await expect(mic).toBeVisible();
    await mic.click();
    await expect(page.locator("#vm-status")).toContainText(/听写未配置|听写服务不可用/, { timeout: 10_000 });
  });
