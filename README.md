...
**现状总览见 [docs/architecture-overview.md](docs/architecture-overview.md)。** 完整架构图见 [docs/diagrams/architecture-complete.png](docs/diagrams/architecture-complete.png)。设计蓝图与原则见 [docs/architecture.md](docs/architecture.md)。若要把 Agent loop 从槽里拆成独立进程、对标 Cursor 现行形态并用 AgentScope Java，见 [docs/agentscope-java-loop-plan.md](docs/agentscope-java-loop-plan.md)（路径）和 [docs/agentscope-java-loop-design.md](docs/agentscope-java-loop-design.md)（工程设计；实现在 `services/neo-loop`，开关 `AGENT_KERNEL=agentscope`）。三态落地规格（This Computer / Cloud / Remote，含控制面 WSS 反代）见 [docs/server-side-agent-loop.md](docs/server-side-agent-loop.md)。Web / Desk / Mobile 三端怎么对齐 Cursor、共享客户端核心和多端实测问题清单见 [docs/client-unification.md](docs/client-unification.md)。后管与 New API 怎么拆见 [docs/admin-platform-research.md](docs/admin-platform-research.md)。现网入口要不要换成 Nginx：不要，见 [docs/nginx-research.md](docs/nginx-research.md)。WorkBuddy 专家 / 专家团怎么跟见 [docs/workbuddy-experts.md](docs/workbuddy-experts.md)。Codex / WorkBuddy 技能与插件市场怎么跟见 [docs/skill-plugin-marketplace.md](docs/skill-plugin-marketplace.md)。骨架齐了之后 WorkBuddy 还值得跟什么见 [docs/workbuddy-feature-gap-2026-08.md](docs/workbuddy-feature-gap-2026-08.md)。跨 Run 的 Agent 记忆接谁、不接谁见 [docs/agent-memory-research.md](docs/agent-memory-research.md)；记忆现在怎么跑、要不要让用户改见 [docs/memory-edit-analysis.md](docs/memory-edit-analysis.md)，怎么做见 [docs/memory-edit-plan.md](docs/memory-edit-plan.md)。Browser-use / computer-use 调研见 [docs/browser-computer-use.md](docs/browser-computer-use.md)。现网两台轻量不要混：应用机（北京 `62.234.211.200`，部署本仓库）见 [.cursor/skills/tencent-lighthouse-deploy/SKILL.md](.cursor/skills/tencent-lighthouse-deploy/SKILL.md)；库机（`101.42.105.230`，Docker MySQL / Redis）见 [.cursor/skills/tencent-lighthouse-db/SKILL.md](.cursor/skills/tencent-lighthouse-db/SKILL.md)。域名 `neorun.cloud` 的解析与 HTTPS 见 [docs/production-domain.md](docs/production-domain.md) 和 [.cursor/skills/tencent-lighthouse-domain/SKILL.md](.cursor/skills/tencent-lighthouse-domain/SKILL.md)。

## 怎么拆

**一个 monorepo，四个控制面进程，一张 worker 镜像。** 不要按模块开仓库。库机上的 New API 是 Gateway 上游，不是第五个 Neo 进程。`neo-loop` 是 Java AgentScope loop（`:8082` 仅内网）；现网默认 `AGENT_KERNEL=pi`。要走 Java 就显式传 `kernel:"agentscope"` 并启动 `neo-loop`。
...
neo-cloud-agent/
  ...
  services/neo-loop         进程 4：Java AgentScope loop（:8082 仅内网，可选）
...
终端客户端（设计见 [docs/cli.md](docs/cli.md)）走同一套 `/v1`，不在本机跑 pi：

```bash
pnpm neo --repo fixtures/toy-repo -p "只回复一个词：pong。不要调用工具。"
pnpm neo login --email admin --password 123456
pnpm neo ls
pnpm neo log <runId>
```

`NEO_API_URL` 默认 `http://127.0.0.1:8080`。CI 用 `NEO_API_KEY` 或 `--api-key`。`--output-format json|stream-json` 给脚本；`--detach` 只创建不阻塞。
