...
- 对外 `/v1` 用用户 session（`POST /v1/auth/login`，用户名或手机号）或 `CONTROL_PLANE_TOKEN`。`POST /v1/auth/register` 用手机号注册（另需用户名和密码，无验证码，手机号唯一）。默认必须登录（`ACCOUNTS_REQUIRED=0` 才允许匿名）。默认管理员仍是 `admin` / `123456`，登录时查账号库（接了 MySQL 就查 `users` 表）。对话页不预填、不跳过。Worker 走 `/internal`，只带 run JWT。`/health`、静态页和公开 webhook 不需要令牌。
