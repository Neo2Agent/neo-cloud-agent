...
5. **不要为了「推理在云端」把 Agent 放控制面。** 推理走 Gateway 即可。若要把 loop 从 VM 拆到独立进程（对标 Cursor 后来的 Temporal 形态），那是第四个进程，不是把 `HarnessAgent` 嵌进控制面；路径见 [agentscope-java-loop-plan.md](./agentscope-java-loop-plan.md)，接口与包结构见 [agentscope-java-loop-design.md](./agentscope-java-loop-design.md)。
