...
- DeepSeek：`deepseek-v4-flash` 是默认便宜模型；`deepseek-chat` / `deepseek-reasoner` 已退役，读写设置时改写成 flash。`deepseek-v4-pro` 显式保留。对话页可以切 Flash / Flash 4.1 预览 / Pro。预览 id `deepseek-v4.1-flash-expires-on-0910` 原生多模态，贴图时不要改写成 vision-exp。
