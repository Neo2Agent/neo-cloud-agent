...
3. 配额：同时跑的对话 / 本月 token 已落地（`GET /v1/quota`）；完整账务仍后置。请求限流已落地：控制面按 IP / 登录 / 用户写操作 / SSE 并发，Gateway 按 run JWT 与 org；`GET /v1/rate-limits` 看当前桶。设了 `REDIS_URL` 后计数走 Redis 固定窗口，否则进程内 token bucket。`RATE_LIMIT=0` 关闭。
