...
- 对外 `/v1` 用用户 session（`POST /v1/auth/register|login`）或 `CONTROL_PLANE_TOKEN`。`ACCOUNTS_REQUIRED=1` 时必须登录。设了 `BOOTSTRAP_EMAIL` / `BOOTSTRAP_PASSWORD` 后启动会创建这个账号。Worker 走 `/internal`，只带 run JWT。`/health` 和静态页不需要令牌。对话页左下角可以登录；未强制登录时可以先跳过。
