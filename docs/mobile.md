...
`packages/web` 已经按窄屏做了侧栏、登录、Enter 发送策略和 `visualViewport`。现网是 `http://neorun.cloud/`（IP 仍是 `http://62.234.211.200/`）。域名绑定见 [production-domain.md](./production-domain.md)。
...
域名已经绑好：`http://neorun.cloud/`（Caddy `:80` → `:8080`，IP 仍是 `62.234.211.200`）。解析步骤见 [production-domain.md](./production-domain.md)。**HTTPS 还没开**，所以对商店包仍不行：

- iOS App Transport Security 默认拒绝明文 HTTP
- Android 9+ 同样默认清掉 cleartext
- APNs / FCM 的 webhook、Universal Link / App Link 都要真实 HTTPS 域名

上架或 TestFlight 真机之前，控制面必须有 TLS。Caddy 已经在应用机上，用 Let's Encrypt **不用花钱**，不要为了 App 再开一个网关，也不要买腾讯云付费证书。不要点轻量控制台一键 HTTPS（只支持应用镜像）。
