...
- 个人记忆：`GET|POST /v1/memories`、`POST /v1/memories/search`、`PATCH|DELETE /v1/memories/:id`、`GET|PATCH /v1/settings/memory`（只要召回开关）；抽屉进「记忆」。Mem0 没配就显示 `configured: false`，不当报错。没有钉住 / 提升 / 用户规则
