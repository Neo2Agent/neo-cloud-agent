...
> **2026-09 现状（已落地，纠正调研里的过期设想）：** 个人记忆是全局可检索的 Mem0 条目。记忆页看 / 记 / 改 / 删；任何对话开场用第一句检索，命中的写入 `.neo/MEMORY.md`。项目侧只有人手写的系统提示词（`Project.instruction` = `PROJECT.md`），只在该项目里的对话生效，不进日常对话。没有个人规则层、没有钉住、没有「提升为规则 / 挪到项目」，也不把项目事实再做一份向量库。下文里「语义记忆还缺」「项目事实也按 projectId 进向量库」是调研时的方向，不是现网产品。
