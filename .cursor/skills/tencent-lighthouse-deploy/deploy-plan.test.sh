...
OUT="$(printf '%s\n' "services/neo-loop/src/main/java/cloud/neorun/loop/LoopApplication.java" | bash "$PLAN")"
expect loop "$OUT" "build_loop=1"
expect loop "$OUT" "restart_loop=1"
expect loop "$OUT" "install=0"
expect loop "$OUT" "build_web=0"
expect loop "$OUT" "restart_control_plane=0"
pass "neo-loop source rebuilds the jar and does not restart Node units"

OUT="$(printf '%s\n' ".cursor/skills/tencent-lighthouse-deploy/units/neo-loop.service" | bash "$PLAN")"
expect loop-unit "$OUT" "update_units=1"
expect loop-unit "$OUT" "restart_loop=1"
expect loop-unit "$OUT" "build_loop=1"
expect loop-unit "$OUT" "restart_control_plane=0"
pass "neo-loop unit file updates systemd without restarting Node units"

OUT="$(printf '%s\n' "services/neo-loop/target/neo-loop-0.1.0.jar" | bash "$PLAN")"
expect loop-jar "$OUT" "build_loop=0"
expect loop-jar "$OUT" "restart_loop=0"
pass "neo-loop Maven target is ignored"
...
pass "--full rebuilds the optional neo-loop jar but does not restart it"

HEALTH="$ROOT/.cursor/skills/tencent-lighthouse-deploy/deploy-health.py"
SAMPLE="$(printf '%s\n' "active" "active" "active" "---" \
  '{"ok":true,"service":"control-plane","workerRuntime":"vm","agentKernel":"pi","vmSlots":{"total":2},"metadataStore":"mysql","eventBus":"redis","llmConfigured":true}' \
...
pass "health summarizer accepts a pi-kernel probe without neo-loop"

WITH_LOOP="$(printf '%s\n' "$SAMPLE" '{"ok":true,"service":"neo-loop"}')"
printf '%s\n' "$WITH_LOOP" | python3 "$HEALTH" >/dev/null
pass "health summarizer still accepts an optional neo-loop probe"
