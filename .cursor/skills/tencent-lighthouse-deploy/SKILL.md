...
ssh lighthouse '
  ...
  curl -sS --max-time 2 http://127.0.0.1:8082/health || echo loop_down
...
| 项 | 现状 |
| --- | --- |
| 进程 | `neo-llm-gateway` `:8081`，`neo-control-plane` `:8080`，`neo-admin-api` `:8090`（本机），可选 `neo-loop` `:8082` 仅 127.0.0.1，Caddy `:80` + `:443` |
...
```bash
ssh lighthouse 'journalctl -u neo-control-plane -u neo-llm-gateway -u neo-admin-api -u neo-loop -n 80 --no-pager'
```
...
- 改完代码照常 commit、push，再 `bash .cursor/skills/tencent-lighthouse-deploy/deploy.sh`。脚本自己决定重启哪个 unit，并保证现网 `AGENT_KERNEL=pi`。**不要**把 `:8082` 写进 Caddy。
